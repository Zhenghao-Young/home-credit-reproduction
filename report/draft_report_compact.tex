...
折安全是核心不变量（表~\ref{tab:foldsafe}）：每个群组聚合都只在当前训练折内拟合，对验证折或
...
的 private 最高）。因此本文把官方分数作为外部有效性检查，归因以固定 folds 下 OOF 配对对比
为主。
...
\begin{table}[h]
\centering\small
\caption{S5 与两种 stacking 方法的 OOF AUC。}\label{tab:rq3-stages}
\begin{tabular}{lrrr}
\toprule
方法 & OOF AUC & $\Delta$ vs S5 & 同向折数 \\
\midrule
S5（最优单模型）          & 0.786342 & ---           & --- \\
